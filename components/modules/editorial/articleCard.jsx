...
const ArticleCard = ({
  ...
  return (
    ...
    >
      <h3 className="text-3xl m-0 p-0 whitespace-normal max-md:text-2xl">{title}</h3>
      <div className="pb-2 mt-5 max-md:mt-4 w-full flex justify-between items-end border-b border-black text-sm max-md:text-xs">
        <span className='translate-y-[1px] max-md:translate-y-0'>{category}</span>
        <span className="font-serif italic text-base max-md:text-sm leading-none max-md:translate-y-[2px]">{timeRead}</span>
      </div>
      <div
